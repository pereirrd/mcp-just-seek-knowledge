# /criar_base_conhecimento — Criar base de conhecimento do workspace

Você deve **criar uma base de conhecimento completa** sobre o projeto/produto usando **todo o workspace aberto** no Cursor (todos os projetos e diretórios disponíveis), lendo documentação e análises já existentes, e então **persistindo o resultado** no MCP **`mcp-just-seek-knowledge`** usando a tool **`ingest`**.
...
- **Preferência por `ingest`**: grave a base usando `mcp-just-seek-knowledge.ingest`. Se o `service_name` já existir e a ingestão falhar, use `mcp-just-seek-knowledge.update` apenas como fallback.
...
2) Faça **uma única** chamada para persistir a base de conhecimento do workspace inteiro:

- Tool: **`mcp-just-seek-knowledge.ingest`**
- Arguments:
...
3) Se `ingest` falhar por `service_name` já existir:

- Use **`mcp-just-seek-knowledge.update`** para atualizar o registro existente, mantendo o mesmo `service_name`.

## Saída final esperada

- Uma chamada de `ingest` contendo uma base de conhecimento completa e reutilizável para o **workspace inteiro**.
